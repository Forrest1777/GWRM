...
const tools = ["get_project_info", "run_project", "get_debug_output", "stop_project", "get_godot_version", "create_scene", "launch_editor"];
let launchSerial = 0;
rl.on("line", (line) => {
  ...
  if (msg.method === "initialize") write(msg.id, { protocolVersion: msg.params.protocolVersion, capabilities: { tools: {} }, serverInfo: { name: "fake", version: "1" } });
    ...
    if (name === "launch_editor") {
      launchSerial += 1;
      const pid = 700000000 + (process.pid * 100) + launchSerial;
      const payload = { name, args, pid, editor_pid: pid };
      write(msg.id, {
    ...
    write(msg.id, { content: [{ type: "text", text: JSON.stringify({ name, args }) }] });
  }
  else write(msg.id, {});
});
