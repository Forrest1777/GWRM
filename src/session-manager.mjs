...
import { isPidAlive, terminateProcessTree } from "./process-utils.mjs";
...
export class SessionManager {
  ...
  async #stopRuntime(record, reason) {
    ...
    if (!runtime && !record.godot_pid && !record.godot_mcp_pid) {
      record.status = record.desired_active ? record.status : "stopped";
      await this.#persist(record);
      return;
    }
