...
try:
    if not workspace:
        raise RuntimeError("HERMES_KANBAN_WORKSPACE is missing")
    real = str(pathlib.Path(workspace).resolve())
    result["workspace"] = real
    if not real.startswith("/workspace/"):
        raise RuntimeError("workspace is outside /workspace")
    if not pathlib.Path(real).is_dir():
    ...
    result.update({
        ...
        "initial_git_clean": not bool(status.strip()),
        "project_godot_present": pathlib.Path(workspace, "project.godot").is_file(),
        "agents_md_present": pathlib.Path(workspace, "AGENTS.md").is_file(),
    })
    ...
    result["sparse_checkout"] = sparse_checkout
    result["sparse_index"] = sparse_index
    ...
    result["missing_tracked_files"] = missing[:200]
    if not branch:
        result["errors"].append("worktree is detached or has no branch")
    ...
    checks = [
        inside == "true", linked_worktree, top == workspace, bool(branch), not status.strip(),
        not sparse_checkout, not sparse_index, not result["skip_worktree_entries"], not missing,
        result["project_godot_present"], result["agents_md_present"],
    ]
